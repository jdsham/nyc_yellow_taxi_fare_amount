...
def run_ridge(X_train:np.array, y_train:np.array, X_test:np.array, y_test:np.array, args:argparse.ArgumentParser, base_path:str, run_name:str, feature_names:list, metrics:dict, artifacts:list) -> tuple:
    ...
    model = Ridge(alpha=best_alpha, random_state=args.random_state)
    model.fit(X_train, y_train)
    ...
    # Learning Curve
    artifacts = plot_learning_curve(Ridge(**model_params), X_train, y_train, model_name, run_name, artifacts, metric="rmse")
    ###################
    # </Metric Curves>
    ###################
